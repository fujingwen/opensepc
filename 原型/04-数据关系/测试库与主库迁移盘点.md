...
- `test.t_project`：3746
- `master.t_project`：当前工程项目主表

结论：

- 工程项目主链路应统一维护在 `master.t_project`
- 历史数据以 `test.t_project -> master.t_project` 为迁移方向
...
本节只核对迁移清单涉及的表关系，并对 `openspec/原型/04-数据关系` 现有文档进行纠偏。
...
2. 代码回调清单
   - 工程项目接口、页面、SQL 是否都已统一到 `t_project`
