...
建材填报模块的字典数据需要从旧系统 `test` schema 迁移到新系统 `master` schema。当前 `master` 中存在少量手动创建的字典数据，部分编码可能与旧系统冲突，需要先检查并标记冲突，再做增量迁移。
...
先检查目标库中已有 `dict_type` / `dict_type + dict_value` 冲突，再执行增量迁移；不删除已有字典，避免误删无法追溯的数据。
...
1. `sql/migrate/cleanup_old_dict.sql` — 输出冲突检查结果（不删除数据）
2. `sql/migrate/migrate_dict.sql` — 迁移字典类型和数据，跳过已有编码
