...
13. 新增编辑的弹窗，在一体化工程编码的select选择框之下新增一行注释，注释内容是“注：可使用施工许可证号或工程名称过滤”
14. 新增时，施工许可证、施工许可证发证日期不是必填；质量监督机构需要确认；施工单位是openspec\changes\archive\2026-04-01-base-companyinfo提案下的施工单位，新增时不要disabled，是下拉；有无填报和是否对接一体化平台编码字段新增时不需要；
15. 操作按钮中，不需要“批量删除”按钮和列表的批量选择，增加”导出”和”导出已开工未填报项目表”这两个按钮。导出已开工未填报项目表指的是工程进度中除了施工前准备阶段和土方开挖及基坑支护阶段的其他阶段。导出的表名称叫“工程信息管理”，表字段是：工程名称、施工许可证、工程进度、工程地址、施工单位、施工单位现场负责人、施工单位现场负责人联系方式、质量监督机构、填报状态
