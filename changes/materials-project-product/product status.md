...
- 审核复用详情弹窗；审核态字段全部 disabled，右上角提供“审核通过 / 审核不通过”按钮
- “审核不通过”通过次级弹窗填写不通过类别与原因
- hover 提示按代理商、生产单位分组展示公司、联系人、电话；若某一方不通过，则不通过类别与原因跟随展示在该企业块下方
- 不通过类别展示优先解析 `sys_dict(shbtgyylb)`，若业务字段保存的是历史 `base_dictionarydata.id` 也必须正确映射
