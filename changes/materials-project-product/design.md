...
- 代理商登录后进入建材产品列表时，筛选条件应以“当前代理商归属的 `supplier_id`”为主，而不是以“当前待审核状态”作为列表可见性的前置条件。
- 因此代理商应能查看本代理商名下全部建材产品，包括：
  - 待审核
  - 审核通过后已流转或已完成的记录
  - 审核不通过、待再次审核、再次审核完成的记录
- “是否允许审核”仍由当前状态决定：
  - 仅代理商当前处于待审核 / 待再次审核阶段的记录可执行审核
  - 已审核完成的记录只能查看，不能再次审核
- 生产单位登录视角仍保持现有“按本单位可处理范围查看”的收口逻辑，不因本次代理商调整而扩大。
...
- “超 48 小时未审核”提醒不是循环提醒，而是按当前待审核阶段只提醒一次。
- 系统通过 `check_out_time` 一类的阶段提醒标记控制幂等，避免同一待审核阶段每次调度都重复发消息。
- 当记录进入新的待审核阶段时，需重置该标记并重新开始计时，典型场景包括：
  - 新增建材产品后进入首轮待审核
  - 审核不通过后由施工单位重提，进入待再次审核
  - 代理商审核通过后流转到生产单位待审核
- 因此系统的语义是“每个待审核阶段超过 48 小时最多提醒 1 次”，而不是“每隔 48 小时固定提醒 1 次”。
- 仅不同企业块之间保留分隔线
...
## Audit Message Design Addendum

### 4.2.4 审核消息与超时提醒

- 本变更不改造独立的消息中心页面或公告模块，但建材产品审核链路必须接入现有站内消息能力。
- 消息类型统一使用站内消息表中的 `message_type = warning`；待审核、待再次审核、审核结果提醒使用 `business_type = audit`，超时提醒使用 `business_type = timeout`。
- 施工单位新增建材产品时：
  - 有代理商时，系统向代理商发送待审核消息。
  - 无代理商时，系统向生产单位发送待确认消息。
- 施工单位在“不通过后重提”时：
  - 有代理商时，系统向代理商发送待再次审核消息。
  - 无代理商时，系统向生产单位发送待再次确认消息。
- 代理商审核通过后，除继续通知生产单位确认外，系统还要向施工单位发送一条审核结果消息。
- 代理商审核不通过后，系统也要向施工单位发送一条审核结果消息，便于施工单位根据不通过原因处理并再次提交。
- 超时提醒按“当前待处理审核方”发出：
  - 代理商待处理超过 48 小时，向代理商发送超时提醒。
  - 无代理商或代理商已审核通过后，若生产单位待处理超过 48 小时，向生产单位发送超时提醒。
- 右上角铃铛读取统一提醒摘要接口，因此上述审核消息和超时提醒都必须能出现在铃铛列表中，并可跳转到对应建材产品。
- `check_out_time` 字段用于标记是否已触发超时提醒：
  - 新增、重提时重置为“否”。
  - 代理商审核通过并流转到生产单位阶段时重置为“否”。
  - 发送超时提醒后置为“是”，避免重复发送。
