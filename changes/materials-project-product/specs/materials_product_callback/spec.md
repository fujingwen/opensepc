...
#### Scenario: 代理商可查看名下全部建材产品

- **Given** 当前登录用户为代理商
- **And** 建材产品的 `supplier_id` 属于该代理商
- **When** 代理商进入建材产品列表页面
- **Then** 系统应返回该代理商名下全部建材产品记录
- **And** 不得仅因为记录已审核完成、审核不通过或已流转到生产单位阶段而从列表中过滤掉

#### Scenario: 代理商仅对当前待审核阶段记录执行审核

- **Given** 当前登录用户为代理商
- **When** 代理商查看本代理商名下的建材产品列表
- **Then** 仅当前处于代理商待审核或待再次审核阶段的记录可执行审核操作
- **And** 其余已审核或已流转记录仅可查看，不可再次审核
