...
### Requirement: 审核消息必须使用站内消息

系统 SHALL 使用站内消息而非公告承载建材产品审核期间的待办、结果与超时提醒。
#### Scenario: 新增或重提后通知当前审核方

- **Given** 施工单位新增建材产品，或在不通过后重新提交
- **When** 系统完成保存
- **Then** 若存在代理商，系统向代理商发送 `business_type = audit` 的站内消息
- **And** 若未选择代理商，系统直接向生产单位发送对应的待确认消息

#### Scenario: 代理商审核结果通知施工单位

- **Given** 代理商对建材产品执行“审核通过”或“审核不通过”
- **When** 后端完成状态更新
- **Then** 系统必须向施工单位发送 1 条 `business_type = audit` 的站内消息
- **And** 该消息可从右上角铃铛和消息中心进入

#### Scenario: 当前待处理审核方超过 48 小时时发送超时提醒

- **Given** 当前建材产品仍处于待审核或待再次审核状态
- **When** 当前待处理方超过 48 小时仍未完成审核
- **Then** 系统必须向当前待处理审核方发送 1 条 `business_type = timeout` 的超时提醒消息
- **And** 该消息应可以在统一提醒入口中跳转到对应的建材产品

#### Scenario: 同一待审核阶段的超时提醒只发送一次

- **Given** 某条建材产品在当前待审核阶段已经发送过 1 次超时提醒
- **When** SnailJob 后续再次扫描到同一待审核阶段且审核状态未发生流转
- **Then** 系统不得重复发送同一阶段的超时提醒消息

#### Scenario: 进入新的待审核阶段后重新计时

- **Given** 某条建材产品已经完成上一待审核阶段的超时提醒或审核流转
- **When** 记录因重提或审核流转进入新的待审核阶段
- **Then** 系统必须重新开始计算 48 小时超时窗口
- **And** 在新的待审核阶段内，系统最多再次发送 1 条 `business_type = timeout` 的超时提醒消息
