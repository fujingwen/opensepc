...
### Requirement: 管理员不受企业角色限制

系统 SHALL 允许超级管理员或租户管理员直接维护采购/库存公告，而不受施工企业或生产企业角色限制。

#### Scenario: 超级管理员保存采购公告草稿

- **Given** 当前登录用户为超级管理员
- **When** 用户创建采购公告并保存为草稿
- **Then** 系统允许保存

#### Scenario: 租户管理员发布库存公告

- **Given** 当前登录用户为租户管理员
- **When** 用户发布库存公告
- **Then** 系统允许发布

### Requirement: 发布后发送目标受众通知

系统 SHALL 在采购/库存公告首次发布时向目标受众发送站内消息通知。

#### Scenario: 发布库存公告后通知施工企业用户

- **Given** 一条库存公告从 `draft` 切换为 `published`
- **When** 系统完成公告发布
- **Then** 系统向施工企业用户发送库存公告站内消息

#### Scenario: 发布采购公告后通知生产企业用户

- **Given** 一条采购公告从 `draft` 切换为 `published`
- **When** 系统完成公告发布
- **Then** 系统向生产企业用户发送采购公告站内消息
