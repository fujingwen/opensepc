...
- `publisher_user_id/publisher_name` 作为公告来源标识，在列表和详情页直接展示
...
- 采购/库存公告从草稿切换为已发布时，系统向目标受众发送站内消息
- 消息发送异步执行，避免阻塞公告发布接口
- 收件箱记录按批量写入方式落库，降低逐条插入带来的超时风险
...
- 列表页提供查询、新建、编辑草稿、删除草稿、发布草稿、查看详情
- 列表页展示发布人，便于识别公告来源
- 表单页当前提供的主动作是保存草稿：
  - `确定`（保存为草稿）
  - `取消`
- 正式发布入口保留在列表页草稿行的 `发布` 按钮
- 新建或编辑时只要求填写：
  - 标题
  - 内容
- 不再展示企业下拉选择项
...
2. 采购/库存公告的可见范围依赖角色、企业上下文和发布状态组合判断，后续如果新增角色或菜单权限，需要同步评估权限边界。
